document.getElementById("transaction-form").addEventListener('submit', function(e) {
    ...
    const saldoAtual = calcularSaldo(); // Adicione esta linha para obter o saldo atual

    if (tipo === "2" && valor > saldoAtual) {
        // Se for uma transação de saque e o valor exceder o saldo atual, exiba um alerta
        const confirmacao = confirm("Aviso: Esta transação resultará em um saldo negativo. Deseja continuar?");
        if (!confirmacao) {
            return; // Interrompe a execução se o usuário optar por não prosseguir
        }
    }

    data.transactions.unshift({
        value: valor, type: tipo, description: descricao, date: data
    });
